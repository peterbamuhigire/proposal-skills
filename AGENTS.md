...
- Book extractions and book summaries must never be stored in this repository, in any folder or under any name (owner rule, 2026-09-23; the former `book-extractions/` folder was removed). Book knowledge lands only as task-oriented `SKILL.md` and `references/` guidance with a short citation (author, year, title, publisher); see `skills/writing-content/premium-commercial-writing/references/copyright-safe-source-use.md`.
- References must not be single-book digests. Organise every `references/` file by this engine's task or workflow, synthesise across sources in the engine's own order and wording, and apply named frameworks with brief attribution. Do not reproduce a book's lists in its sequence with its item titles, its case studies, or near-verbatim text (owner rule, 2026-09-24).
...
- Proposal sections: use the numbered section skills under `skills/pipeline/01-` through `skills/pipeline/10-`
...
- Retail transformation and retail software proposals: use `skills/domain-delivery/retail-transformation-proposal/` when the assignment includes retail, omnichannel, e-commerce operations, POS, merchandising, pricing, promotions, markdowns, loyalty, CRM, fulfilment, returns, store operations, shrink, vendor funding, private label, planogram, new-store opening, or retail KPI/WBR work.
...
- Proposal to marketing plan (business-plan-skills): when a proposal sells a marketing plan, hand over the brief, objectives, budget ceiling, markets and evidence gaps; the plan structure and SMART objectives come back for the methodology.
- Proposal to digital marketing and advertising (social-media-skills): hand over approved scope, audience, channels, measurement expectations and commercial boundaries; channel strategy, campaign method and reporting design come back. Never promise reach, engagement or conversion figures without data.
- Proposal to website delivery: for website-only or website-led work, hand off approved scope, purpose statement, asset ownership register, care-plan commitments, discovery notes, sitemap/content/SEO assumptions, design rationale, launch commitments, acceptance criteria, support package, and commercial boundaries to the website engine.
...
- If you modify or add skills, keep frontmatter limited to `name`, `description`, and portable `metadata` as defined by `docs/skill-authoring-standard.md`; keep the body execution-oriented and move heavy detail into directly linked `references/`.

## July 2026 authoring and release gate

- The active catalogue is every `SKILL.md` under `skills/`, including `skills/SKILL.md`; discover it from the filesystem and keep the documented count aligned with machine output.
- Follow [the local authoring standard](docs/skill-authoring-standard.md) and start new work from [the skill template](templates/skill-template.md).
...
Every proposal, bid, EOI, cover letter, executive summary, methodology, case study, and evaluator-facing message must load the [human-English editorial standard](skills/language/language-standards/references/human-english-editorial-standard.md) and the [proposal phrase bank](skills/writing-content/premium-commercial-writing/references/proposal-phrase-bank.md) alongside the profile, ToR, section skill, critical-analysis, and anti-slop gates. Apply its five passes and preserve evaluator-specific register, proof, compliance language, and honest delivery constraints.
