...
description: Use when a proposal concerns banking, insurance, payments, mobile money, microfinance, digital finance, or financial inclusion; also apply the finance doctrine whenever money, ledger, tax, or accounting claims are in scope.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
1. Confirm the sector fit from the brief.
2. Read the sector reference file for evidence, terminology, and current signals.
3. Translate the sector context into problem framing, methodology choices, stakeholder logic, and risks.
4. Use the sector framing inside the relevant numbered proposal section skills.
...
|---|---|---|
| Sector-specific framing and methodology cues for financial-sector assignments. | Proposal lead, section writer, and evaluator-readiness reviewer | Applied consistently, traceable to the selected source, and free of unsupported identity, framework, or sector claims. |
...
|---|---|---|
| Regulatory and accounting boundary | Name the product, regulator, jurisdiction, reporting framework, money flow, and required professional review before making claims. | Unverified compliance or unsafe financial-system assurance |
...
A solicitation requires **regulatory and accounting boundary**, but one controlling input is missing. The proposal team applies this rule: Name the product, regulator, jurisdiction, reporting framework, money flow, and required professional review before making claims. It records the gap and owner, withholds the affected assurance, and proceeds only with content that can be verified. This avoids **unverified compliance or unsafe financial-system assurance**.
...
## Finance Quality Gate Record

| Field | Result |
|---|---|
| Gate and doctrine | Finance & Accounting Quality Gate; doctrine v1.0.0 |
| Scope assessed | Structural normalisation of this proposal-routing skill; no rates, postings, financial statements, statutory treatment, or production system changes |
| Blockers | None found in the changed skill |
| Required-check status | Doctrine and gate linked; final client artefacts still require current-source verification and named professional review where applicable |
| Release state | `pass-with-caveats` for this internal skill artefact |
