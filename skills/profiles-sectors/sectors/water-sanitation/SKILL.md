...
description: Use when a proposal concerns WASH programmes, sanitation services, water utilities, hygiene behaviour change, or community water systems; use health when clinical or public-health delivery is primary.
metadata:
  portable: true
  compatible_with:
  - claude-code
...
|---|---|---|
| Sector-specific framing and methodology cues for WASH assignments. | Proposal lead, section writer, and evaluator-readiness reviewer | Applied consistently, traceable to the selected source, and free of unsupported identity, framework, or sector claims. |
...
|---|---|---|
| Service-chain boundary | Map source, treatment, distribution or containment, service provider, user behaviour, safeguards, and O&M ownership. | Infrastructure-only response with no sustainable service model |
...
A solicitation requires **service-chain boundary**, but one controlling input is missing. The proposal team applies this rule: Map source, treatment, distribution or containment, service provider, user behaviour, safeguards, and O&M ownership. It records the gap and owner, withholds the affected assurance, and proceeds only with content that can be verified. This avoids **infrastructure-only response with no sustainable service model**.

## References

- [Proposal skill router](../../../SKILL.md) — orchestration, profile, reasoning, and release gates.
<!-- dual-compat-end -->
- [references/uganda-wash-sector.md](references/uganda-wash-sector.md) for local sector context.
